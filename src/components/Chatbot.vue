...
<template>
    <div class="fixed bottom-0 right-10 text-white">
        <div class="w-[400px] font-bold bg-neutral-800">
            <div class="p-4 font-bold cursor-pointer border-b border-neutral-600 flex items-center gap-3"
                @click="handleToggleChat">
                <img :src="'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQrjVfM-36dfcntAoGw_Xguj2RdQ6MQFttDKw&s'"
            ...
            <div v-if="isShowChat" class="h-[400px] flex flex-col">
                <div class="flex-1 flex flex-col  gap-1 py-3 px-5 overflow-y-scroll">
                    <div v-for="message of messages"
                        :class="'flex gap-3 items-center ' + (message.from === 'user' ? 'self-end' : '')">
                ...
                <div class="p-2 flex gap-2">
                    <input type="text" class="input bg-white text-black flex-1" :disabled="isLoading"
                        v-model="messageInput">
                    <button :class="messageInput ? 'cursor-pointer' : 'cursor-not-allowed'" @click="handleSendMessage"
                        :disabled="isLoading">
