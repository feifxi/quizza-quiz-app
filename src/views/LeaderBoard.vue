...
<template>
  ...
  <div class="min-h-screen bg-gradient-to-b from-green-100 to-white p-6">
    <header class="flex justify-center gap-4 mb-6 relative">
      ...
      <Button label="👍 Like" class="bg-pink-400 border-b-4 border-pink-600 active:bg-pink-600"
    ...
    <section class="max-w-3xl mx-auto">
      ...
      <div v-if="changeTab" class="transition-all duration-300">
        <div v-for="(user, index) in sortUsersByStar" :key="user.id"
          ...
          <div
            class="border-1 rounded-full w-13.5 h-13.5 mx-auto flex items-center justify-center relative border-black"
            :style="{
              backgroundImage: 'linear-gradient(to ' + (user.profileFrame?.selectedMode || 'top') + ','
            ...
            <img :src="user.profilePic ||
              ...
              " alt="" class="w-11 h-11 rounded-full object-cover absolute border-1 border-black" />
          </div>
          <div class="flex-1">
